...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->kabupaten == '00') {
            $total = DB::table('monitoring')
                ->select(DB::raw('"00" kabupaten, "SULAWESI UTARA" nama_kabupaten, sum(jumlah_entri) as jml_entri'));
    ...
    public function kab($kab)
    {
        if (auth()->user()->kabupaten == '00') {
            ...
            $total = DB::table('monitoring')
                ->select(DB::raw('"000" kabupaten,' . '"' . $kabupaten->nama_kabupaten . '"' . ' nama_kabupaten, sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $kabupaten->kabupaten);

            $data = DB::table('monitoring')
                ->select('kecamatan', 'nama_kecamatan', DB::raw('sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $kabupaten->kabupaten)
    ...
    public function kec($kab, $kec)
    {
        if (auth()->user()->kabupaten == '00') {
            ...
            $total = DB::table('monitoring')
                ->select(DB::raw('"000" kecamatan,' . '"' . $kecamatan->nama_kecamatan . '"' . ' nama_kecamatan, sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $kecamatan->kabupaten)
                ->where('kecamatan', '=', $kecamatan->kecamatan);

            $data = DB::table('monitoring')
                ->select('desa', 'nama_desa', DB::raw('sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $kecamatan->kabupaten)
                ->where('kecamatan', '=', $kecamatan->kecamatan)
    ...
    public function desa($kab, $kec, $desa)
    {
        if (auth()->user()->kabupaten == '00') {
            ...
            $total = DB::table('monitoring')
                ->select(DB::raw('"000" desa,' . '"' . $desakel->nama_desa . '"' . ' nama_desa, sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $desakel->kabupaten)
                ->where('kecamatan', '=', $desakel->kecamatan)
                ->where('desa', '=', $desakel->desa);

            $data = DB::table('monitoring')
                ->select('nbs', 'nks', DB::raw('sum(jumlah_entri) as jml_entri'))
                ->where('kabupaten', '=', $desakel->kabupaten)
                ->where('kecamatan', '=', $desakel->kecamatan)
                ->where('desa', '=', $desakel->desa)
                ->groupBy('nbs', 'nks')
